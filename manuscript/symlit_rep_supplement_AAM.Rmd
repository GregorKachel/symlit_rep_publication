...
## Study 1
Posterior predictive checks (PPC) for both full and null models indicated
...
Comparing the models using weights based on the Widely Applicable Information Criterion (WAIC) yielded `r round(S1.weights["S1.full.bm","value"]*100,2)`% of the model weight for the full model, and `r round(S1.weights["S1.null.bm","value"]*100,2)`% for the null model. Hence, the full model generally had a higher probability of making accurate predictions. Directly comparing the models' WAIC via expected log predictive density (ELPD) corroborates this (ELPD WAIC; full model = `r round(S1.comp["S1.full.bm", "elpd_waic"],2)`; null model = `r round(S1.comp["S1.null.bm", "elpd_waic"],2)`). The difference in predictive accuracy (ELPD diff = `r round(S1.comp["S1.null.bm", "elpd_diff"],2)`) was smaller than its standard error (SE = `r round(S1.comp["S1.null.bm", "se_diff"],2)`), indicating that the evidence in favor of the full model is not decisive. A similar comparison via Leave-One-Out Cross-Validation (LOO) provided essentially the same results. In the absence of conclusive evidence for either model, we report the results for the full model in line with the preregistration.

## Study 2

For both full and null models, PPCs indicate excellent fit of observed
...
analyses using the full model are reported below.

## Study 3

PPCs indicated excellent fit of observed data and model predictions in both models. Rhat values in both models were equal to one, indicating convergence across all chains. Effective sample sizes for all fixed
...
remains within the range of sampling uncertainty (SE =
...
each study. For this we added crossed random effects for items,
allowing the relationship between age and performance to vary across
items (correct \~ condition\*z.age + z.trial + sex + (z.trial \| subid) +(z.age \| item)).
...
question. However, the analyses provide detail about the equivalence of
...
Model weights based on the WAIC suggested that the full model was more likely to provide accurate predictions (weight = `r round(S1.itemvsfull.weights["S1.full.bm","value"]*100,2)`%) compared to the item model (weight = `r round(S1.itemvsfull.weights["S1.item.model","value"]*100,2)`%). The difference in expected log predictive density was negligible (ELPD diff = `r round(S1.itemvsfull.comp["S1.item.model", "elpd_diff"],2)`), and within the range of sampling uncertainty (SE =`r round(S1.itemvsfull.comp["S1.item.model", "se_diff"],2)`). For an overview of log odds for all items across conditions see Figure \@ref(fig:plot-study123itemintercepts). For a full overview of coefficients and the random effects structure of the item model see Table \@ref(tab:S1item-coef-table).

## Study 2

An exploratory model including item-level random effects indicated modest variability ($\beta$ = `r S2.item.coef["sd_item__Intercept", "mean"]`, 95% CrI [`r S2.item.coef["sd_item__Intercept", "q2.5"]`,`r S2.item.coef["sd_item__Intercept", "q97.5"]`]). The variability of age effects across items was small ($\beta$ = `r S2.item.coef["sd_item__z.age", "mean"]`, 95% CrI [`r S2.item.coef["sd_item__z.age", "q2.5"]`, `r S2.item.coef["sd_item__z.age", "q97.5"]`]), suggesting that the effect of age was fairly consistent across items. The correlation between item intercepts and age slopes was near zero and highly uncertain (ρ = `r S2.item.coef["cor_item__Intercept__z.age", "mean"]`, 95% CrI [`r S2.item.coef["cor_item__Intercept__z.age", "q2.5"]`,`r S2.item.coef["cor_item__Intercept__z.age", "q97.5"]`]), providing no reliable evidence that slightly easier or more difficult items varied in age-related gains. 
...
Graphically exploring item-level variability via log odds indicates
